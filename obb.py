...
def create_obb_box(center, axes, extents, name="obb_box"):
    ...
    for i in [-1, 1]:
        for j in [-1, 1]:
            for k in [-1, 1]:
                corner = (center + i * extents[0] * axes[0] +
                          j * extents[1] * axes[1] +
                          k * extents[2] * axes[2])
    ...
    box_cgo = [
        LINEWIDTH, 2.0,
        BEGIN, LINES,
        COLOR, 1.0, 1.0, 0.0  # Yellow color for the box lines
    ]
...
cage = Cage()
cage.load("MTC1.mol2")
cage.detect_cavity(step=0.6, probe_in=1.4, probe_out=12, removal_distance=2.4, volume_cutoff=200)
...
layers = cage._get_cluster_layers("Pd", factor=0.2)
cluster = FaceCenteredCubic(
                    symbols="Pd",
                    surfaces=surfaces,
                    layers=layers,
                    latticeconstant=3.8901,
                )
cluster.positions += cage.centroid
# %%

source_center, source_axes, _ = calculate_obb(cluster.positions)

rotation_matrix = np.dot(axes.T, source_axes)

translated_source_points = cluster.positions - source_center

rotated_source_points = np.dot(translated_source_points, rotation_matrix)

aligned_points = rotated_source_points + center

cluster.positions = aligned_points
# %%
